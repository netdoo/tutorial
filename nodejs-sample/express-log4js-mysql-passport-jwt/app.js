...
const memberFindSQL = 'SELECT member_no, name, level, pass, is_active FROM tb_member WHERE name = ?';
...
passport.use('local-login', new localStrategy({
  ...
  passReqToCallback: true //인증을 수행하는 인증 함수로 HTTP request를 그대로  전달할지 여부를 결정한다
    ...
    pool.getConnection().then(function(connection) {
        dbconn = connection;
        return dbconn.query(memberFindSQL, username);
    }).then(function(rows) {
        if (rows.length === 0) {
...
/// [주의]
/// connect-ensure-login 모듈을 사용하여 인증 여부를 확인하게 될 경우, 
/// JWT 형태로 전달되는 TOKEN 검사는 제외된다.
app.get('/private', login.ensureLoggedIn('/static/login.html'), function(req, res) {
    res.send(req.user);
});
